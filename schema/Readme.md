...
Type mappings bridge Prisma's data types with the datasource's data types. Type mappings also provide a way to choose the transportation format.
...
model Post {
  id        Int             @id
  title     ms.VarChar(255)
  wordCount ms.SmallInt
}
```

`User` has implicit PostgreSQL scoping due to `default = true`. Post uses `ms.` in front of `ms.VarChar(255)` & `ms.SmallInt` to explicitly choose the MySQL datatypes.
