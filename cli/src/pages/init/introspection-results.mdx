...
For greenfield projects, we offer a selection of [examples](https://github.com/prisma/photonjs/tree/master/examples) to the user that they can select during `prisma2 init`. For brownfield project, we eventually want to get to a similar level of sophistication and e.g. offer a to auto-generate a GraphQL server that's based on the existing database schema of the user.
...
Use similar setup as in `script` example:
