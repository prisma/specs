...
1. Display all available examples (screens: [SQLite 4](./sqlite#4-example-selection), [MySQL 5](./mysql#5-example-selection), [PostgreSQL 5](./postgresql#5-example-selection))
1. Success screens (screens: SQLite 5, MySQL 6, PostgreSQL 6)
  ...
  "typescript": {
    ...
    "grpc": {
      ...
      "nextStepInstructions": [{
        ...
        "text": "Use a gRPC client script, e.g.:",
        "commands": ["npm run feed # fetch all posts", "npm run signupUser # create new user (hardcoded)"]
      }]
    },
    // ...
  }
...
Alternatively, it could be implemented via a GraphQL API that's based on a similar JSON file.