...
> **Note**: We previously also wanted to run an initial migration using `prisma lift save` and `prisma lift up` but decided against it so that users get their example in a "blank" state. The initial migration is also not needed because the ouput includes the instruction to immediately run `prisma dev` which auto-migrates the DB.

Steps 1-3 are hardcoded in the CLI based on the following assumptions that all example projects (no matter what kind of examples are being added in the future) always:

- will have a `prisma.schema` with a `datasource` configuration

Steps 4-6 are performed dynamically per example and are included in the Prisma example API. The setup commands are included in the JSON data returend by the examples API under the `setupCommands` key.
