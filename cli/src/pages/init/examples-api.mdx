...
Steps 1-4 are hardcoded in the CLI based on the following assumptions that all example projects (no matter what kind of examples are being added in the future) always:

- will have a `prisma.schema` with a `datasource` configuration
- are using Lift

Steps 5-7 are performed dynamically per example and are included in the Prisma example API. The setup commands are included in the JSON data returend by the examples API under the `setupCommands` key.
  ...
  "examples": {
    "javascript": {
      "graphql": {
        ...
        "setupCommands": ["npm install", "prisma2 generate", "npm run seed"],
