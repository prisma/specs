...
- Use schema `public`
  - [`public` is empty](../postgresql#10-language-selection)
...
<Terminal>
  ...
  <BorderBox
    flexDirection="column"
    title={`${chalk.bold('Empty schemas')} ${chalk.gray(
      '(Start new project)'
    )}`}
    marginBottom={1}
  >
  ...
  <BorderBox
    flexDirection="column"
    title={`${chalk.bold('Non-empty schemas')} ${chalk.gray(
...
- [Empty schema](../postgresql#10-language-selection)
...
- [Create](../postgresql#10-language-selection)
